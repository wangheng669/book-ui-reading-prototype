...
const interaction = (mode, cta, prompt, options, analysis, blockIds) => ({ mode, cta, prompt, options, analysis, blockIds });
...
const owner = {
  ...
  [bid(15)]: { needed: true, reason: "比较结论同时受实际收益、长期结果和近期价格影响，轻量对照能减少单一指标误判。", componentType: "comparison", componentData: { title: "资产比较的两个时间尺度", sourceBlockIds: range(11, 15), items: [{ label: "长期", text: "考察未来较长时期的实际回报与购买力。", blockIds: [bid(11), bid(12), bid(15)] }, { label: "近期", text: "考察当前价格、收益率以及随后几年可能发生的变化。", blockIds: [bid(13), bid(15)] }], interaction: interaction("perspective", "切换长期与近期视角", "同一项资产，在不同时间尺度下要观察什么？", [{ id: "future-guarantee", text: "未来股票仍会稳定取得同样收益", perspective: "历史结果不能自动成为未来保证，原文强调未来表现高度不确定。" }, { id: "comparison-input", text: "它是比较资产的证据之一，但还要结合当前收益率和时间尺度", perspective: "这保留了历史证据，同时没有忽略当前债券收益率、长期结果和近期体验。" }], "同一个历史数字，在‘描述过去’和‘保证未来’之间存在重要边界。", range(13, 15)) } },
